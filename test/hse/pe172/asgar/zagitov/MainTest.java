...
public class MainTest {
    ...
    public void firstSort() {
        ...
        assertTrue(result[1].contains(String.format(Main.SORT_SORTED_ARRAY, "")));
        assertEquals(result[1].substring(result[1].indexOf(":") + 2), "1 1 2 4 5 7 10 12");
        String allTime = String.format(Main.SORT_ALL_TIME, 0);
        assertTrue(result[2].contains(allTime.substring(0, allTime.indexOf("="))));
    }
    ...
    public void secondSort() {
        ...
        assertTrue(result[3].contains(partTimeFirst.substring(0, partTimeFirst.indexOf("="))));

        String partTimeSecond = String.format(Main.SORT_PART_TIME, 2, 0);
        assertTrue(result[4].contains(partTimeSecond.substring(0, partTimeSecond.indexOf("="))));

        String mergeTime = String.format(Main.SORT_MERGE_TIME, 0);
        assertTrue(result[5].contains(mergeTime.substring(0, mergeTime.indexOf("="))));
    }
    ...
    public void thirdSort() {
        ...
        assertTrue(result[6].contains(sendTime.substring(0, sendTime.indexOf("="))));

        String receiveTime = String.format(Main.SORT_RECEIVE_TIME, 0);
        assertTrue(result[7].contains(receiveTime.substring(0, receiveTime.indexOf("="))));
